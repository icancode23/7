...
public class NavigationView extends ScrimInsetsFrameLayout {
    ...
    class C02621 implements Callback {
        C02621() {
        }
    ...
    public static class SavedState extends AbsSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C02631());
        public Bundle menuState;

        static class C02631 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C02631() {
            }
    ...
    public NavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0003R.styleable.NavigationView, defStyleAttr, C0003R.style.Widget_Design_NavigationView);
        ViewCompat.setBackground(this, a.getDrawable(C0003R.styleable.NavigationView_android_background));
        if (a.hasValue(C0003R.styleable.NavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a.getDimensionPixelSize(C0003R.styleable.NavigationView_elevation, 0));
        }
        ViewCompat.setFitsSystemWindows(this, a.getBoolean(C0003R.styleable.NavigationView_android_fitsSystemWindows, false));
        this.mMaxWidth = a.getDimensionPixelSize(C0003R.styleable.NavigationView_android_maxWidth, 0);
        if (a.hasValue(C0003R.styleable.NavigationView_itemIconTint)) {
            itemIconTint = a.getColorStateList(C0003R.styleable.NavigationView_itemIconTint);
        } else {
            itemIconTint = createDefaultColorStateList(16842808);
        }
        boolean textAppearanceSet = false;
        int textAppearance = 0;
        if (a.hasValue(C0003R.styleable.NavigationView_itemTextAppearance)) {
            textAppearance = a.getResourceId(C0003R.styleable.NavigationView_itemTextAppearance, 0);
            textAppearanceSet = true;
        }
        ColorStateList itemTextColor = null;
        if (a.hasValue(C0003R.styleable.NavigationView_itemTextColor)) {
            itemTextColor = a.getColorStateList(C0003R.styleable.NavigationView_itemTextColor);
        }
        if (!textAppearanceSet && itemTextColor == null) {
            itemTextColor = createDefaultColorStateList(16842806);
        }
        Drawable itemBackground = a.getDrawable(C0003R.styleable.NavigationView_itemBackground);
        this.mMenu.setCallback(new C02621());
        ...
        if (a.hasValue(C0003R.styleable.NavigationView_menu)) {
            inflateMenu(a.getResourceId(C0003R.styleable.NavigationView_menu, 0));
        }
        if (a.hasValue(C0003R.styleable.NavigationView_headerLayout)) {
            inflateHeaderView(a.getResourceId(C0003R.styleable.NavigationView_headerLayout, 0));
        }
        a.recycle();
    }
    ...
    private ColorStateList createDefaultColorStateList(int baseColorThemeAttr) {
        ...
        if (!getContext().getTheme().resolveAttribute(C0163R.attr.colorPrimary, value, true)) {
            return null;
        }
